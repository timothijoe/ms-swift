#!/bin/sh
# =============================================================================
# CoC Trace Driving GRPO — Debug: Guided Force Trigger (CoC enabled, prob=1.0)
# =============================================================================
# 用于 VSCode Debug 的强制触发入口脚本。
# 启用 CoC Trace 引导，probability=1.0（每组 prompt 必然触发引导）。
# 适合调试引导生成逻辑、替换逻辑、logprobs 保留等机制。
#
# 用法：
#   bash coc_trace_rl/scripts/debug/run_guided_force.sh
#
# 可覆盖的环境变量：
...
"${PYTHON_BIN}" -m coc_trace_rl.src.coc_trace_main \
  ...
  --output_dir "${PROJECT_ROOT}/output/COC_TRACE_DEBUG_GUIDED_FORCE" \
  --max_length 512 \
