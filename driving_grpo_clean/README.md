...
- `code/`: 训练与奖励相关代码（入口、参数、预处理、reward plugin）。
- `instructions/`: RM 模板与前缀指令文件。
- `data/`: manifest 配置文件。

## 运行

```bash
bash driving_grpo_clean/code/run_driving_grpo_manifest.sh
```

可覆盖的常用环境变量：

```bash
CUDA_VISIBLE_DEVICES=0 NPROC_PER_NODE=1 bash driving_grpo_clean/code/run_driving_grpo_manifest.sh
```

如果你的环境只有 `python` 没有 `python3`：

```bash
PYTHON=python bash driving_grpo_clean/code/run_driving_grpo_manifest.sh
```
...
`data/datasets_manifest.json` 使用相对路径 `my_data/driving_sample_32.jsonl`。样本 JSONL 中如果还有旧路径 `/home/linux/project/v3.12-ms-swift/...`，预处理器会在读取样本时映射到当前仓库根目录。

## 如果要跑 mixed

把 `data/datasets_manifest.json` 里的 `preprocess_type` 改成 `driving_mixed`，并把脚本里的 reward 改成：
...
- `--external_plugins driving_grpo_clean/code/driving_rm_plugin.py`
